// scraper-server.js
import puppeteer from 'puppeteer-extra'; // Import Browser type
...
function removeLastWordRegex(str: string): string {
  if (!str) {
...
async function scrapeUltimateGuitar(page: Page, ugUrl: string) {
...
const server = http.createServer(async (req, res) => {
  ...
  const reqUrl = new URL(currentReqUrl, `http://${req.headers.host}`);
  ...
  if (req.method === 'POST' && reqUrl.pathname === '/scrape') {
    ...
    req.on('end', async () => {
      let requestedUrlForErrorContext: string | undefined;
      try {
